...
impl Fixture<'_> {
    fn new() -> Pin<Box<Self>> {
        ...
        unsafe {
            ...
            boxed.as_mut().get_unchecked_mut().photoprism_container =
                Some(photoprism_container);
        };

        boxed
    }
    fn get_photoprism_url(&self) -> String {
        let port = self.photoprism_container.as_ref().unwrap().get_host_port_ipv4(2342);
...
async fn test_add_photo_add_labels() -> Result<(), anyhow::Error> {
    with_fixture(|fixture: &Fixture| async {
        ...
        let file_path = concat!(env!("CARGO_MANIFEST_DIR"), "/resources/tests/low_quality_photo.jpg");
...
async fn test_refresh_token() -> Result<(), anyhow::Error> {
    with_fixture(|fixture: &Fixture| async {
        ...
        let user_before_reload = photoprism_service
            .get_user().await?;
        let mut retries = 10;
        loop {
            retries -= 1;
            sleep(Duration::from_secs(1)).await;
            let user_after_reload = photoprism_service
                .get_user().await?;

            if user_before_reload.session_id != user_after_reload.session_id {
                debug!("old session id: {}, new session id: {}", user_before_reload.session_id, user_after_reload.session_id);
                break;
            }

            if retries == 0 {
                Err(anyhow!("Session id has not been refreshing during 10 seconds"))?;
            }
        }
